...
sukisupatchesdir="$outside/SukiSU/patches/$(echo $kernel_ver | cut -d. -f1,2)"
...
if [[ -d "$sukisupatchesdir" ]]; then
  for patch_file in "$sukisupatchesdir"/*.patch ; do
    patch -p1 < "$patch_file"
  done
else
  echo "patching SukiSU-Ultra failed, the kernel version you want to patch doesnt have patches here yet"
  exit 1
fi
